  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">Gallery</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{pagination ? `${pagination.total} photos` : ""}</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {photos.map((photo) => (
            <div key={photo.id} className="group relative bg-white rounded-md border border-zinc-200 dark:border-zinc-700 overflow-hidden">
              ...
              <div className="p-3">
                ...
                <span className={`inline-flex mt-1 px-2 py-0.5 rounded-full text-xs font-medium ${photo.is_published ? "bg-green-100 text-green-700" : "bg-zinc-100 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500"}`}>
                  {photo.is_published ? "Published" : "Draft"}
                </span>
              </div>
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                <Link href={`/gallery/${photo.id}`} className="px-2 py-1 bg-white text-xs font-medium text-sxc-navy rounded-md shadow hover:bg-zinc-50 dark:hover:bg-zinc-800 dark:hover:bg-zinc-800">Edit</Link>
