  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">Members</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{pagination ? `${pagination.total} members` : ""}</p>
        </div>
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {members.map((member) => (
                <tr key={member.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      {member.photo_url ? (
                        ...
                        <div className="w-8 h-8 rounded-full bg-zinc-200 flex items-center justify-center shrink-0">
                          <span className="text-xs font-medium text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{member.name[0]}</span>
                        </div>
                      )}
                      <span className="font-medium text-zinc-800 dark:text-zinc-200">{member.name}</span>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2 py-0.5 text-xs font-medium ${member.role_type === "executive" ? "bg-sxc-navy text-white" : member.role_type === "management" ? "bg-sxc-blue text-white" : "bg-zinc-100 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500"}`} style={{ borderRadius: "3px" }}>
                      {member.role_type}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{member.position ?? "—"}</td>
                  <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{member.period ?? "—"}</td>
                  <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{member.sort_order}</td>
                  <td className="px-4 py-3">
