  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-zinc-900 dark:text-zinc-100">News</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{pagination ? `${pagination.total} articles found` : ""}</p>
        </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-2">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search news..." className="flex-1 px-3 py-2 text-sm border border-zinc-200 dark:border-zinc-700 rounded-md focus:outline-none focus:ring-2 focus:ring-sxc-blue" />
            ...
            <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {news.map((item) => (
                <tr key={item.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                  ...
                  <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{item.author ?? "—"}</td>
                  <td className="px-4 py-3 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500">{formatDate(item.published_at)}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${item.is_published ? "bg-green-100 text-green-700" : "bg-zinc-100 text-zinc-500 dark:text-zinc-400 dark:text-zinc-500"}`}>
